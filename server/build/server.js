...
app.get('/weapon-select', function (req, res) {
  db.query('SELECT *  FROM `weapon` where weapon_id = ?', [req.query.id], function (err, results) {
    if (err) {
      return res.send(err);
    } else {
      return res.json({
...
app.get('/monster-select', function (req, res) {
  db.query('SELECT *  FROM `monster_part` where name_id = ?', [req.query.id], function (err, results) {
    if (err) {
      return res.send(err);
    } else {
      return res.json({
...
app.post('/weapon', function (req, res) {
  ...
  if (req.body.class == 0) {
    ...
    db.query('SELECT * FROM `weapon` w \
      ...
      WHERE wl.name = ?', [req.body.class], function (err, results, fields) {
      if (err) {
        return res.send(err);
      } else {
        return res.json({ data: results });
...
app.get('/hitzone', function (req, res) {
  ...
  db.query('SELECT * FROM `monster_part` WHERE name_id = ?', [req.query.name], function (err, results) {
    if (err) {
      return res.send(err);
    } else {
      console.log(results);
